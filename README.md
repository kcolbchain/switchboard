...
| `switchboard/gas_budget.py` (✅ shipped, `gas_tracker.py` deprecated) | ✅ shipped (PR [#14](https://github.com/kcolbchain/switchboard/pull/14)) | **Hard gas budgets** (per-hour, per-day) so an agent can't get rugged by its own runaway loop. Closes the #1 footgun of autonomous on-chain agents. `GasBudgetTracker` is the blessed API — per-wallet, rolling-window, thread-safe. The older `GasTracker` singleton in `gas_tracker.py` still works but emits a `DeprecationWarning` and will be removed in v0.3; migrate via `GasBudgetTracker(...).bind_wallet(addr)` for the single-wallet `can_send_transaction` / `record_gas_usage` API. |
